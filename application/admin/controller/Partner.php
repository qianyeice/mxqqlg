...
use app\admin\model\Founder_partner;
...
class Partner extends adminController {
    ...
    public function index(){
        ...
        $data = new Founder_partner();
        $sou=!is_null(input('keyword')) ? input('keyword') : null;
        $type = !is_null(input('type')) ? input('type') : 1;
        $start = !is_null(input('page')) ? input('page')+1 : 1;
        $limit = !is_null(input('limit')) ? input('limit') :20;
        $table = $data->To_partner_authorization($start,$limit,$type,$sou);
